      ...
      <el-form ref="applicationRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="证书类型" prop="certType">
          <el-select v-model="form.certType" placeholder="请选择证书类型">
            <el-option
              v-for="dict in project_certificate_type"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
        ...
        <el-form-item label="证书价格" prop="certPrice">
          <el-input v-model="form.certPrice" placeholder="请输入证书价格" />
        </el-form-item>
...
const data = reactive({
  ...
  rules: {
    ...
    certPrice: [
      { required: true, message: "证书价格不能为空", trigger: "blur" }
    ],
    payStatus: [
      { required: true, message: "支付状态：0-未支付，1-已支付，2-退款中，3-已退款不能为空", trigger: "change" }
    ],
    createBy: [
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除证书申请编号为"' + _ids + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('project/application/export', {
    ...queryParams.value
